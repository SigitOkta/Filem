...
class MovieFragment :
            ...
            override fun onMenuItemSelected(item: MenuItem): Boolean {
                return when (item.itemId) {
                    R.id.menu_now_playing -> {
                        setActionBarTitle(viewModel.getFilterTitle(MovieFilter.NOW_PLAYING))
                        setupRecyclerView(MovieFilter.NOW_PLAYING)
                        true
                    }
                    R.id.menu_upcoming -> {
                        setActionBarTitle(viewModel.getFilterTitle(MovieFilter.UPCOMING))
                        setupRecyclerView(MovieFilter.UPCOMING)
                        true
                    }
                    else -> false
                }
    ...
    private fun setupRecyclerView(filter : MovieFilter) {
        val recyclerView = binding.rvGrid
        ...
        lifecycleScope.launch {
            viewModel.getFilteredMovies(filter).collectLatest {
